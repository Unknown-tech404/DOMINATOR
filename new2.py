...
{Fore.YELLOW}{Style.BRIGHT}FEATURES:{Style.RESET_ALL}
    ...
    {Fore.CYAN}•{Style.RESET_ALL} Port scanning {Fore.LIGHTBLACK_EX}(25 common ports with banner grabbing){Style.RESET_ALL}
...
{Fore.YELLOW}{Style.BRIGHT}REQUIREMENTS:{Style.RESET_ALL}
    {Fore.MAGENTA}pip install dnspython python-whois requests aiohttp colorama{Style.RESET_ALL}
...
class Dominator:
        ...
        for ip in list(self.ips)[:20]:
            try:
                ...
                if open_ports:
                    ...
                    color_print(f"[+] {ip}: {len(open_ports)} open ports", Fore.GREEN)
                else:
    ...
    def generate_report(self):
        ...
        for ip, ports in self.results.get("port_scan", {}).items():
            ...
            color_print(f"{ip}: {port_list}", Fore.GREEN)
